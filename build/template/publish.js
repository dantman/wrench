...
function publish(symbolSet) {
	...
	IO.copyFile(publish.conf.templatesDir+"static/default.css", publish.conf.outDir);
...
function includeTemplate(path) {
	var path = publish.conf.templatesDir+path;
	try {
		var template = new JSDOC.JsPlate(path);
	}
	catch(e) { print(e.message); quit(); }
...
function resolveLinks(str, from) {
	str = str.replace(/\{@link ([^} ]+) ?\}/gi,
		function(match, symbolName) {
			if( /^https?:\/\//i.test(symbolName) )
				return '<a rel=nofollow href="' + symbolName + '">' + symbolName + '</a>';
...
function linkTo(item) {
	if( /^https?:\/\//i.test(item) )
		return '<a rel=nofollow href="' + item + '">' + item + '</a>';
	return new Link().toSymbol(item);
}
